...
namespace Wpf.Core
{
    public static class AnimationHelper
    {
        ...
        public static DoubleAnimation CreateDoubleAnimation(string propertyName, double to, double duration, double decelerateRatio = 0, double beginTime = 0)
        {
            var a = new DoubleAnimation(to, new Duration(TimeSpan.FromSeconds(duration)));
            a.DecelerationRatio = decelerateRatio;
            a.BeginTime = TimeSpan.FromSeconds(beginTime);
            Storyboard.SetTargetProperty(a, new PropertyPath(propertyName));
            return a;
